...
(function (global, factory) {
        ...
        bufferSync(t) {
            ...
            if (t.dtype === 'string') {
                try {
                    ...
                    const strings = data.map(d => tfjsCore.util.decodeString(d));
    ...
    function castImpl(values, shape, inputType, dtype) {
        ...
        if (dtype === 'bool') {
            ...
            const zero = tfjsCore.util.toTypedArray([0], inputType);
    ...
    const bitwiseAnd = binaryKernelFunc(tfjsCore.BitwiseAnd, bitwiseAndImpl);
    ...
    function createSimpleUnaryImpl(op) {
        return (values, dtype, attrs) => {
            const newValues = tfjsCore.util.getArrayFromDType(dtype, values.length);
    ...
    function validateIndices(indices, indicesShape, numParams) {
        indices.forEach((index, i) => {
            if (index < 0 || index >= numParams) {
                const locString = tfjsCore.util.indexToLoc(i, indicesShape.length, tfjsCore.util.computeStrides(indicesShape))
                    .join(',');
    ...
    function getSplits(outSplits) {
        ...
        for (let i = 0; i < outSplits.length; ++i) {
            ...
            const splits = tfjsCore.util.getArrayFromDType('int32', numSplits);
    ...
    function getValues(paramsDenseValues, paramsDenseValuesShape, paramsDenseValuesDType, valueSlices, numValues) {
        ...
        const valuesOut = tfjsCore.util.getArrayFromDType(paramsDenseValuesDType, tfjsCore.util.sizeFromShape(valuesShape));
    ...
    function raggedRangeImpl(starts, startsShape, startsDType, limits, limitsShape, deltas, deltasShape) {
        ...
        const rtNestedSplits = tfjsCore.util.getArrayFromDType('int32', nRows + 1);
        rtNestedSplits[0] = 0;
        ...
        const rtDenseValues = tfjsCore.util.getArrayFromDType(startsDType, nVals);
        let valueIndex = 0;
        for (let row = 0; row < nRows; ++row) {
    ...
    class RaggedTensorToTensorOp {
        ...
        calculateOutputSize(firstDim) {
            ...
            const outputShape = tfjsCore.backend_util.combineRaggedTensorToTensorShapes(this.raggedRank, shape, valueShape);
        ...
        calculateFirstParentOutputIndex(firstDimension, outputIndexMultiplier, firstDimensionOutput) {
            ...
            tfjsCore.util.assert(result.length === firstDimension, () => 'Final length of result must be equal to firstDimension.');
            return result;
        }
        ...
        compute() {
            ...
            const outputTensor = tfjsCore.util.getArrayFromDType(this.valuesDType, tfjsCore.util.sizeFromShape(outputShape));
        ...
        setOutput(raggedRank, outputIndex, outputTensor, outputShape) {
            ...
            const valueElementSize = tfjsCore.util.sizeFromShape(elementShape);
            ...
            if (defaultValue.length !== valueElementSize && defaultValue.length !== 1) {
                ...
                tfjsCore.tidy(() => {
                    const defaultValueTensor = tfjsCore.reshape(defaultValue, srcShape);
                    const bCastDefault = tfjsCore.broadcastTo(defaultValueTensor, elementShape);
    ...
    function scatterImpl(indices, updates, shape, outputSize, sliceSize, numUpdates, sliceRank, strides, defaultValue, sumDupeIndices) {
        ...
        const outBuf = (defaultValue instanceof tfjsCore.TensorBuffer) ?
            defaultValue :
    ...
    const staticRegexReplace = unaryKernelFuncFromImpl(tfjsCore.StaticRegexReplace, staticRegexReplaceImpl);
    ...
    function searchSortedImpl(sortedInputs, values, batchSize, numInputs, numValues, side) {
        const output = tfjsCore.util.getArrayFromDType('int32', batchSize * numValues);
        for (let b = 0; b < batchSize; ++b) {
    ...
    function sparseToDense(args) {
        ...
        switch (sparseValues.dtype) {
            ...
            case 'string': {
                const updatesBuf = backend.bufferSync(sparseValues);
                const $defaultValue = tfjsCore.util.decodeString(backend.data.get(defaultValue.dataId).values[0]);
                outBuf = scatterImpl(indicesBuf, updatesBuf, outputShape, outputSize, sliceSize, numUpdates, sliceRank, strides, $defaultValue, sumDupeIndices);
                break;
            }
            default:
    ...
    function tensorScatterUpdate(args) {
        ...
        const { sliceRank, numUpdates, sliceSize, strides, outputSize } = tfjsCore.backend_util.calculateShapes(updates, indices, tensor.shape);
        const sumDupeIndices = false;
        const indicesBuf = backend.bufferSync(indices);
        const updatesBuf = backend.bufferSync(updates);
    ...
    function transform(args) {
        ...
        const inStrides = tfjsCore.util.computeStrides(image.shape);
        const batchInStride = inStrides[0];
        const rowInStride = inStrides[1];
        const colInStride = inStrides[2];
        const outStrides = tfjsCore.util.computeStrides(outShape);
